(ns commsdevice.operation
  "OperationActor -- one communication-equipment-manufacturing operation
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (Device Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Radio-Compliance Governor
  ...
  One graph run = one communication-equipment-manufacturing operation
  ...
  decision to a human operator (the radio-compliance engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/ship-device-unit`/`:actuation/issue-radio-
  conformity-certificate` ALWAYS reach this node when the governor is
  clean -- see `commsdevice.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [commsdevice.commsdeviceadvisor :as commsdeviceadvisor]
...
(defn build
  ...
  `commsdevice.store/Store`).
  opts:
    :advisor      -- a `commsdevice.commsdeviceadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (commsdeviceadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Device Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (commsdeviceadvisor/-advise advisor store request)]
            {:proposal p :audit [(commsdeviceadvisor/trace request p)]})))

      ;; Radio-Compliance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
